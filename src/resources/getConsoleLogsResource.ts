    ...
    resources: [
      {
        ...
        name: "All logs",
        description: "All Unity console logs (newest first). Set includeStackTrace=false to save tokens.",
        mimeType: resourceMimeType
      },
      {
        uri: `unity://logs/error?offset=0&limit=20&includeStackTrace=true`,
        name: "Error logs",
        description: "Error logs only. Start with includeStackTrace=false for quick overview.",
        mimeType: resourceMimeType
      },
      {
        uri: `unity://logs/warning?offset=0&limit=30&includeStackTrace=true`,
        name: "Warning logs",
        description: "Warning logs only. Use includeStackTrace=false by default to save tokens.",
        mimeType: resourceMimeType
      },
      {
...
export function registerGetConsoleLogsResource(server: McpServer, mcpUnity: McpUnity, logger: Logger) {
  ...
  server.resource(
    ...
    resourceTemplate,
    {
      description: 'Retrieve Unity console logs by type with pagination support.',
      mimeType: resourceMimeType
    },
    async (uri, variables) => {
      try {

...
export class McpUnity {
  ...
  private async parseAndSetConfig() {
    ...
    this.port = envPort ? parseInt(envPort, 10) : (configPort ? parseInt(configPort, 10) : 8090);
    ...
    // Initialize timeout from environment variable (in seconds) or use default (10 seconds)
